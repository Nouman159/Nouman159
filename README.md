...
Most of my work looks like this: a school needs staff to stop digging through policy PDFs, so I build semantic search over their documents. A coaching platform needs recorded answers scored consistently, so I wire Whisper into GPT-4o and spend most of the time making the scoring trustworthy rather than calling the API.

The model is rarely the hard part. Getting clean data into it, and making the output reliable enough to ship, usually is.
